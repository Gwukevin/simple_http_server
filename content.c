...
{
	const char eresponse[] = "<html><head><title>ERROR</title></head><body><font face=\"sans-serif\"><center><h1>X-P</h1><p>error to content<b>%s</b>.</p></font></center></body>";
	char *resp;
...
content_get(char *path, int *content_len)
{
	...
	if (sanity_check(path) || 
	    ...
	    s.st_size > MAX_CONTENT_SZ) {
			printf("Econtent get:err because of bath||no files or too large-> %lld...\n",s.st_size);
			goto err;
		}

	content_fd = open(path, O_RDONLY);
	if (content_fd < 0) {
		printf("content get:path=%s err because of bath||no files or too large-> %lld...\n",path,s.st_size);
		goto err;
	}
	...
	// printf("\ncontent_get: the s.st_size=\n%lld\nthe amnt-read=\n%d\n end of it",s.st_size,amnt_read);
	*content_len = s.st_size;
	// printf("\ncontent_get: the resp\n%s\n end of it",resp);
	close(content_fd);
	return resp;
err_free:
	free(resp);
